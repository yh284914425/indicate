(() => {
  ...
  var CryptoService = class {
    ...
    // 获取K线数据
    async getKlines(symbol, interval, limit = 1e3, startTime, endTime) {
      try {
        const params = {
          symbol,
          interval,
        ...
        const response = await axios_default.get(`${this.baseUrl}/api/v3/klines`, { params });
    ...
    // 获取更多K线数据
    async getExtendedKlines(symbol, interval, desiredLimit = 2e3) {
      try {
        const batchSize = 1e3;
    ...
    calculateIndicators(klines) {
      const high = klines.map((k2) => parseFloat(k2.high));
      const low = klines.map((k2) => parseFloat(k2.low));
      const close = klines.map((k2) => parseFloat(k2.close));
      ...
      const topDivergence = new Array(klines.length).fill(false);
      const bottomDivergence = new Array(klines.length).fill(false);
      for (let i = 34; i < klines.length; i++) {
        ...
        if (jCrossUpJ1) {
          ...
          for (let k2 = i - 1; k2 >= 34; k2--) {
            if (this.CROSS(j[k2 - 1], j1[k2 - 1], j[k2], j1[k2])) {
              lastCrossIndex = k2;
              break;
            }
          }
          if (lastCrossIndex !== -1) {
            if (close[lastCrossIndex] > close[i] && j[i] > j[lastCrossIndex] && j[i] < 20) {
        ...
        if (j1CrossUpJ) {
          let lastCrossIndex = -1;
          for (let k2 = i - 1; k2 >= 34; k2--) {
            if (this.CROSS(j1[k2 - 1], j[k2 - 1], j1[k2], j[k2])) {
              lastCrossIndex = k2;
              break;
            }
          }
          if (lastCrossIndex !== -1) {
            if (close[lastCrossIndex] < close[i] && j1[lastCrossIndex] > j1[i] && j[i] > 90) {
    ...
    async getSymbols() {
      try {
        ...
        return response.data.symbols.filter((symbol) => symbol.status === "TRADING" && symbol.quoteAsset === "USDT").map((symbol) => symbol.symbol);
      } catch (error) {
  ...
  var config = {
    ...
    // 主要币种
    periods: ["15m", "30m", "1h", "2h", "4h", "6h", "12h", "1d"],
    // 分析周期
    days: 166,
    // 分析天数
    telegram: {
  ...
  var sentSignals = /* @__PURE__ */ new Set();
  ...
  async function analyzePeriod(symbol, interval, limit) {
    try {
  ...
  async function checkNewSignalsAndNotify(signals) {
    ...
    for (const signal of newSignals) {
      const divergenceType = signal.type === "top" ? "\u{1F534}\u9876\u80CC\u79BB" : "\u{1F7E2}\u5E95\u80CC\u79BB";
      const message = `
${divergenceType}\u4FE1\u53F7\u63D0\u9192\uFF01

\u{1F4CA} \u4EA4\u6613\u5BF9: ${signal.symbol}
\u23F1 \u5468\u671F: ${signal.period}
\u{1F4B0} \u5F53\u524D\u4EF7\u683C: ${signal.price}
\u{1F552} \u4FE1\u53F7\u65F6\u95F4: ${signal.time}
...
`;
  ...
  function initWebSocket(symbols) {
    ...
    ws.on("close", () => {
      ...
      setTimeout(() => initWebSocket(symbols), 5e3);
    });
    ws.on("error", (error) => {
  ...
  async function main() {
    ...
    for (const symbol of config.symbols) {
      for (const period of config.periods) {
        console.log(`\u5206\u6790 ${symbol} ${period} \u5468\u671F\u6570\u636E...`);
        const signals = await analyzePeriod(
          symbol,
          period,
          config.days
